...
public class AnalisiAreeVerdi {
	...
	public static Set<AreaVerde> elaboraAnalisi(FiltroInterventi filtro) {
		...
		if	(filtro.getDataInizio().isPresent() || filtro.getDataFine().isPresent()
				|| filtro.getDescrizione().get().strip().isBlank() || filtro.getDescrizione().equals(null)
		 ...
		 */
			...
			try {
				...
				AreaVerde area = new AreaVerde(ID_Gestionale,quartiere,nome,geoPoint);
				areeVerdi.add(area);
				flag++;
				}
